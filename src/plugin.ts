...
penpot.ui.onMessage<Map<string,string>>((message) => {
  ...
  if(message.get("data-version")=="6"){
    ...
    calenderBar.resize(2850, 675);
    calenderBar.fills = [{fillColor: '#FFFFFF'}];

    const date = penpot.createText("08/01/2025");
    if(date){
      date.x = penpot.viewport.center.x+120;
      date.y = penpot.viewport.center.y+200;
      date.fontSize = '245';
    ...
    if(calenderLogo){
      calenderLogo.x = penpot.viewport.center.x+2230;
      calenderLogo.y = penpot.viewport.center.y+90;
      calenderLogo.resize(475,475);
    } 

    const calenderBox = penpot.createRectangle();
    calenderBox.resize(3250,2900);
    calenderBox.x = penpot.viewport.center.x+1000;
    calenderBox.y = penpot.viewport.center.y+1000;
    calenderBox.fills = [{fillColor: '#FFFFFF'}];
    ...
    if(arrow){ 
      arrow.x = penpot.viewport.center.x+1000+2650;
      arrow.y = penpot.viewport.center.y+1000+100;
    ...
    if(arrow2){
      arrow2.x = penpot.viewport.center.x+1000+300;
      arrow2.y = penpot.viewport.center.y+1000+100;
      arrow2.resize(300,300); 
    }

    const month = penpot.createText("August 2025");
    if(month){
      month.x = penpot.viewport.center.x+1000+900;
      month.y = penpot.viewport.center.y+1000+100;
      month.fontSize = '245';
    ...
    for(let i=0; i<content.length; i++){
      ...
      if(textItem){ 
        textItem.x = penpot.viewport.center.x+1000+xPos;
        textItem.y = penpot.viewport.center.y+1000+yPos;
        textItem.fontSize ='200';
    ...
    const highlightBox = penpot.createRectangle();
    highlightBox.resize(2680,480);
    highlightBox.x = penpot.viewport.center.x+1000+250;
    highlightBox.y = penpot.viewport.center.y+1000+550;
    highlightBox.borderRadius=100;
    highlightBox.fills = [{fillColor: '#b1b2b5', fillOpacity: .2}];
    } 

    const highlightCircle = penpot.createEllipse();
    highlightCircle.x = penpot.viewport.center.x+1000+1850;
    highlightCircle.y = penpot.viewport.center.y+1000+1030;
    highlightCircle.resize(300,300);
    highlightCircle.fills = [{fillColor:'#800000', fillOpacity:.2}]; 
